...
/// Represents a single gemfeed entry
#[derive(Debug)]
pub struct Entry {
    ...
    /// URL for the entry
    ///
    /// Note: not guaranteed to be a full path.
    /// Take this and the parent feed's URL to
    /// construct the entry's full URL.
    pub url: String,
...
pub struct Feed {
    ...
    ///
    /// Combine with entry feeds to construct
    /// full entry URLs.
    pub base_url: String,
